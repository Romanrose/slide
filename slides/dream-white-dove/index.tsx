  ...
  <PageShell page={6} section="Storyboard">
    ...
    <div style={{ height: 540, overflow: 'auto' }}>
      ...
      >
        ...
        <tbody>
          {[
            ['1', '01', '中', '背，平', '固定', '沙发正对电视，CNN战争新闻', '新闻背景音'],
            ['1', '02', '近', '斜侧，平', '摇', '桌上酒瓶烟蒂，乔臂上疤痕', '新闻背景音'],
            ['1', '03a', '特', '正，俯', '固定', '乔惊醒，瞳孔放大，喘粗气', '炮火声+喘气'],
            ['2', '01a', '特', '正，俯', '固定', '乔背靠掩体，头下垂', '炮火声'],
            ['2', '02', '中', '斜侧，平', '固定', '约翰快步走来，半蹲包扎', '战场声+脚步声'],
            ['2', '04', '中', '斜侧，平', '固定', '约翰中弹，扑倒在乔身上', '子弹声+哭声'],
            ['2', '06', '特', '略仰', '过肩固定', '约翰遗言："保护好你自己"', '乔的哭声'],
            ['2', '08', '特', '略俯', '过肩固定', '乔掏出打火机，刻有白鸽痕迹', '摩擦声'],
            ['3', '02', '中', '正，平', '固定', '约翰递来打火机，乔端详', '聊天底噪+鸟叫'],
            ['3', '04', '近', '斜侧', '固定', '乔接过打火机，举向天空', '点烟声'],
            ['3', '06', '—', '—', '固定', '白鸽飞过两人头顶', '白鸽咕咕叫'],
            ['4', '02', '近', '斜侧，平', '固定', '乔摸出打火机，摩挲刻痕', '手指摩擦声'],
            ['4', '04', '—', '—', '固定', '白鸽在地板行走，停驻看镜头', '白鸽咕咕叫'],
          ].map(([scene, shot, size, angle, move, content, sound], i) => (
            <tr
              key={shot + String(i)}
              style={{
            ...
            >
              {[scene, shot, size, angle, move, content, sound].map((val, j) => (
                <td
                  key={j}
                  style={{
                    padding: '10px 14px',
                    fontFamily: j <= 1 ? fonts.mono : j === 5 ? fonts.serif : 'inherit',
                    color: j <= 1 ? palette.accent : palette.text,
                    fontSize: j === 5 ? 19 : 21,
                  }}
                >
                  {val}
